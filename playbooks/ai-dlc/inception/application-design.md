...
**Note**: When `frontend-design` stage runs (UI in scope), it consumes
`component-methods.md` from this stage to define UI data bindings. No logic
change in `application-design.md` is required, but the data bindings table
in `frontend-components.md` (per-unit, CONSTRUCTION) cross-references
`component-methods.md` symbols.
...
> ✅ **Approve & Continue** - Approve design and proceed to **Frontend Design** (if UI in scope) or **Units Generation** (if backend-only)
```
