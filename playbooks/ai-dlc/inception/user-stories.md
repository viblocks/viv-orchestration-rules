...
- **Acceptance Criteria** - Ask about detail level, format, testing approach, and validation methods.
  ALWAYS include this option in the format question, regardless of whether UI is in scope:
    "Given/When/Then (Gherkin-style) format.
     Recommended when the project includes UI components — THEN clauses become inputs to
     the state matrix derived in frontend-design.
     Useful even for backend-only projects as a clear way to express AC.
     Reference: viblocks-ai/aidlc-docs/inception/user-stories/stories.md as a working example."
