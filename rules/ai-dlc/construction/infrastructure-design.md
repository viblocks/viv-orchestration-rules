...
### Step 6b: System-Wide Consolidation (after last unit's Infrastructure Design)

If multiple units share infrastructure, generate `aidlc-docs/construction/shared-infrastructure.md` containing:

1. Complete production topology diagram (all deployed services + infrastructure + connections)
2. Inter-service communication map (ports, protocols, service discovery mechanism)
3. Shared resources inventory (any resource accessed by more than one unit)
4. All container/runtime images with pinned versions
5. External services catalog (third-party APIs, managed services, with access method)

The content is DERIVED from the per-unit infrastructure designs — not from a template. Only include sections relevant to the project.

This document is the **single source of truth for production topology** and becomes a formal deliverable in the CONSTRUCTION → VERIFICATION handoff.
