...
- Phase containers: Use lighter Material colors (INCEPTION: #BBDEFB, CONSTRUCTION: #C8E6C9, VERIFICATION: #E1BEE7, DEPLOYMENT: #FFF59D)
...
flowchart TD
    ...
    subgraph VERIFICATION["🔬 VERIFICATION PHASE"]
        TSD["Test Strategy Design<br/><b>STATUS</b>"]
        TED["Test Environment Design<br/><b>STATUS</b>"]
        TES["Test Environment Setup<br/><b>STATUS</b>"]
        TDG["Test Diagnosis<br/><b>STATUS</b>"]
        TI["Test Implementation<br/><b>STATUS</b>"]
        E2EV["E2E Validation<br/><b>STATUS</b>"]
        CIP["CI Pipeline Implementation<br/><b>STATUS</b>"]
        PRG["Production Readiness Gate<br/><b>STATUS</b>"]
    end
    
    subgraph DEPLOYMENT["🟡 DEPLOYMENT PHASE"]
        DSD["Strategy Design<br/><b>STATUS</b>"]
        CDPD["Pipeline Diagnosis<br/><b>STATUS</b>"]
        CDD["Pipeline Design<br/><b>STATUS</b>"]
        CDI["Pipeline Implementation<br/><b>STATUS</b>"]
        DE["Deployment Execution<br/><b>STATUS</b>"]
        PDV["Post-Deploy Validation<br/><b>STATUS</b>"]
        RVS["Rollback + Sign-off<br/><b>STATUS</b>"]
    end
...
### 🔬 VERIFICATION PHASE
- [ ] Test Strategy Design - EXECUTE (ALWAYS)
  - **Rationale**: Test plan based on what was actually built
- [ ] Test Environment Design - EXECUTE (ALWAYS)
  - **Rationale**: Test environment replicating production topology
- [ ] Test Environment Setup - EXECUTE (ALWAYS)
  - **Rationale**: Implement environment and validate with smoke test
- [ ] Test Diagnosis - EXECUTE (ALWAYS)
  - **Rationale**: Honest assessment of existing test coverage
- [ ] Test Implementation - [EXECUTE/SKIP]
  - **Rationale**: [Close test gaps if diagnosis reveals them]
- [ ] E2E Validation - EXECUTE (ALWAYS)
  - **Rationale**: End-to-end system validation following integration sequence
- [ ] CI Pipeline Implementation - EXECUTE (ALWAYS)
  - **Rationale**: Automate test suite in CI/CD with gates
- [ ] Production Readiness Gate - EXECUTE (ALWAYS)
  - **Rationale**: Final GO/CONDITIONAL-GO/NO-GO verdict

### 🟡 DEPLOYMENT PHASE
- [ ] Strategy Design - EXECUTE (ALWAYS)
  - **Rationale**: Define deployment strategy, risk profile, and rollback criteria
- [ ] Pipeline Diagnosis - [EXECUTE/SKIP] (CONDITIONAL — brownfield only)
  - **Rationale**: Audit existing CD pipeline and identify gaps
- [ ] Pipeline Design - EXECUTE (ALWAYS)
  - **Rationale**: Design CD pipeline architecture, environments, and gates
- [ ] Pipeline Implementation - EXECUTE (ALWAYS)
  - **Rationale**: Implement CI/CD pipeline per design
- [ ] Deployment Execution - EXECUTE (ALWAYS)
  - **Rationale**: Execute first production deployment with full observability
- [ ] Post-Deploy Validation - EXECUTE (ALWAYS)
  - **Rationale**: Validate deployment against production criteria
- [ ] Rollback + Sign-off - EXECUTE (ALWAYS)
  - **Rationale**: Rollback validation and final operational sign-off gate
...
### 🔬 VERIFICATION PHASE
- [ ] Test Strategy Design - EXECUTE
- [ ] Test Environment Design - EXECUTE
- [ ] Test Environment Setup - EXECUTE
- [ ] Test Diagnosis - EXECUTE
- [ ] Test Implementation - [EXECUTE/SKIP]
- [ ] E2E Validation - EXECUTE
- [ ] CI Pipeline Implementation - EXECUTE
- [ ] Production Readiness Gate - EXECUTE

### 🟡 DEPLOYMENT PHASE
- [ ] Strategy Design - EXECUTE
- [ ] Pipeline Diagnosis - [EXECUTE/SKIP]
- [ ] Pipeline Design - EXECUTE
- [ ] Pipeline Implementation - EXECUTE
- [ ] Deployment Execution - EXECUTE
- [ ] Post-Deploy Validation - EXECUTE
- [ ] Rollback + Sign-off - EXECUTE
...
🔬 **VERIFICATION PHASE:**
[N]. Test Strategy Design - *Rationale:* [Why executing]
[N]. Test Environment Design - *Rationale:* [Why executing]
[N]. [Additional stages as applicable]
...
