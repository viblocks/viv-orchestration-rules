...
- 🔬 **VERIFICATION PHASE** - Test Strategy, Validation & Production Readiness (DOES IT WORK)
- 🟡 **DEPLOYMENT PHASE** - Deployment & Monitoring (future expansion)
...
**Focus**: DOES IT WORK and IS IT READY  
...
- Test Implementation (CONDITIONAL — if gaps found)
- E2E Validation (ALWAYS)
- CI Pipeline Implementation (ALWAYS)
- Production Readiness Gate (ALWAYS)

**Outputs**: Test strategy, test environment, test suite, E2E evidence, CI pipeline, production readiness verdict

### DEPLOYMENT PHASE
**Purpose**: Production deployment capability — strategy, pipeline, first deploy, rollback validated  
**Focus**: How to DEPLOY and RUN it  
**Location**: `deployment/` directory

**Stages** (7 stages, per `rules/deployment/*` and `rules/common/process-overview.md:68`):
- Strategy Design (ALWAYS)
- Pipeline Diagnosis (CONDITIONAL — brownfield only)
- Pipeline Design (ALWAYS)
- Pipeline Implementation (ALWAYS)
- Deployment Execution (ALWAYS)
- Post-Deploy Validation (ALWAYS)
- Rollback + Sign-off (ALWAYS)

**Outputs**: deployment-strategy.md, cd-pipeline-diagnosis.md (conditional), cd-pipeline-design.md, cd-pipeline-impl-report.md, deployment-execution-report.md, post-deploy-validation-report.md, rollback-validation-signoff.md
